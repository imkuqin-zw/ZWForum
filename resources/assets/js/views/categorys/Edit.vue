        ...
        <div class="box container-fluid" >
            <form class="form-horizontal"  @submit.prevent="validateBeforeSubmit">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-md-6">
                        <input type="number" class="form-control" v-model='formData.order' name="order"  >
                    </div>
                </div>
                <div class="form-group">
                    <label for="name"  class="col-sm-2 control-label" >是否隐藏话题</label>
    ...
    methods: {
        validateBeforeSubmit() {
            this.$validator.validateAll().then(success => {
                ...
                axios.put(window.Domain + 'api/admin/cate/' + this.$route.params.cateId,this.formData)
                    .then(response => {
        ...
        getTagInfo(){
            axios.get(window.Domain + 'api/admin/cate/' + this.$route.params.cateId)
                .then(response => {
                    let data = response.data.data
                    this.formData.name = data.name
                    this.formData.order = data.order
                    this.formData.is_blocked = data.is_blocked
                    this.formData.is_display = data.is_display
